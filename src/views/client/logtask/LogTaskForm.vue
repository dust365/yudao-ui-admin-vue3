...
<!--        <el-input v-model="formData.operator" placeholder="请输入操作者" />-->
<!--      </el-form-item>-->
...
const formRules = reactive({
  ...
  // operator: [{ required: true, message: '操作者不能为空1', trigger: 'blur' }],
  // operationTime: [{ required: true, message: '操作时间不能为空1', trigger: 'blur' }]
})
...
const submitForm = async () => {
  ...
  try {
   ...
   data.operator = userStore.getUser.nickname
    // data.operationTime = getCurrentFormattedTime()
    console.log('当前时间1：'+new Date().getTime())
    console.log('当前时间3：'+getCurrentFormattedDate())
...
function getCurrentFormattedDate(): Date {
  const formattedString = dateUtil().format('YYYY-MM-DD HH:mm:ss')
